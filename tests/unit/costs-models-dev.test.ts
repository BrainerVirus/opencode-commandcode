...
describe("parseModelsDev + applyModelsDevCosts", () => {
  ...
  test("fills by exact id and skips already priced models", () => {
    const models = [
      model({ id: "google/gemini-3.5-flash", name: "Gemini 3.5 Flash", cost: { input: 0.5, output: 2 } }),
  ...
  test("does not apply models.dev prices to Command Code free SKUs", () => {
    const models = [
      model({ id: "minimax/minimax-m2.7-free", name: "MiniMax M2.7 Free", cost: { input: 0.5, output: 2 } }),
    ]
    const filled = new Set<string>()
  ...
  test("matches the last path segment of a catalog id", () => {
    const models = [model({ id: "Qwen/Qwen3.6-Plus", name: "Qwen 3.6 Plus", cost: { input: 0.5, output: 2 } })]
    const filled = new Set<string>()
